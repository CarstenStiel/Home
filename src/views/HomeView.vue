<template>
  <div class="flex flex-col min-h-screen overflow-hidden">
    <div class="bgDiv">
      <div class="mt-20 w-full">
        <!-- Dies dient nur dazu die Navbar zu berücksichtigen -->
      </div>
      <div class="h-full w-full flex flex-col">
        <div
          class="w-fit lg:text-2xl md:text-md font-extrabold tracking-wide uppercase ml-10 my-48 text-white bg-gradient-to-r from-indigo-600 via-purple-500 to-pink-500 rounded-md p-2"
        >
          Willkommen Zuhause! Hier werden allgemeine Informationen dargestellt.
        </div>
        <div class="w-full flex justify-end">
          <div
            class="w-fit lg:text-2xl md:text-md font-extrabold tracking-wide uppercase mr-10 my-48 text-white bg-gradient-to-r from-indigo-600 via-purple-500 to-pink-500 rounded-md p-2"
          >
            Dies ist ein Platzhalter für spätere Informationen
          </div>
        </div>
      </div>
    </div>
    <section>
      <div class="max-w-7xl mx-auto px-4 sm:px-6">
        <div
          class="py-12 md:py-20 border-t border-gray-100 dark:border-gray-800"
        >
          <div class="max-w-3xl mx-auto text-center pb-12 md:pb-16">
            <div
              class="inline-flex text-sm font-semibold py-1 px-3 m-2 text-primary-600 bg-primary-200 rounded-full mb-4"
            >
              Hier kommen ein paar Informationen!
            </div>
            <p class="text-xl text-gray-600 dark:text-gray-400">
              Ideen für Dinge, die man hier einfügen kann!
            </p>
          </div>
          <div class="grid my-10">
            <div class="md:grid md:grid-cols-12 md:gap-6 items-center">
              <div
                class="max-w-xl md:max-w-none md:w-full mx-auto md:col-span-5 lg:col-span-6 mb-8 md:mb-0 order-first"
              >
                <img
                  src="https://cdn.swiftdesk.io/marketing/assets/images/screenshots/swiftdesk-customer-support-software-shared-inbox.png"
                  width="540"
                  height="405"
                  alt="SwiftDesk - Shared Inbox"
                  class="max-w-full mx-auto md:max-w-none h-auto screenshot"
                />
              </div>
              <div
                class="max-w-xl md:max-w-none md:w-full mx-auto md:col-span-7 lg:col-span-6"
              >
                <div class="md:pr-4 lg:pr-12 xl:pr-16">
                  <div
                    class="font-architects-daughter text-xl text-primary-500 mb-2"
                  >
                    Test
                  </div>
                  <h3 class="h3 mb-3">Test Beschreibung</h3>
                  <p class="text-xl text-gray-700 dark:text-gray-400 mb-4">
                    Hier soll ggf. die minimierte Version von einem Tab
                    erscheinen!
                  </p>
                  <ul class="text-lg text-gray-700 dark:text-gray-400 mb-8">
                    <li class="flex items-center mb-2">
                      <svg
                        viewBox="0 0 12 12"
                        xmlns="http://www.w3.org/2000/svg"
                        class="w-3 h-3 fill-current text-teal-500 mr-2 flex-shrink-0"
                      >
                        <path
                          d="M10.28 2.28L3.989 8.575 1.695 6.28A1 1 0 00.28 7.695l3 3a1 1 0 001.414 0l7-7A1 1 0 0010.28 2.28z"
                        ></path>
                      </svg>
                      <span>TEST</span>
                    </li>
                  </ul>
                  <div
                    class="flex items-center mt-6 text-primary-500 hover:text-primary-600 hover:underline"
                  >
                    <svg class="feather feather-xs">
                      <use
                        xlink:href="/images/svg/feather-sprite.svg#arrow-right-circle"
                      ></use>
                    </svg>
                    <a
                      href="/"
                      title="SwiftDesk - Shared Inboxes"
                      class="ml-2 hover:text-primary-600 hover:underline"
                      >Weiterleitung</a
                    >
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div class="grid my-20">
            <div class="md:grid md:grid-cols-12 md:gap-6 items-center">
              <div
                class="max-w-xl md:max-w-none md:w-full mx-auto md:col-span-5 lg:col-span-6 mb-8 md:mb-0 md:order-1"
              >
                <img
                  src="https://cdn.swiftdesk.io/marketing/assets/images/screenshots/swiftdesk-customer-support-software-shared-inbox.png"
                  width="540"
                  height="405"
                  alt="SwiftDesk - Shared Inbox"
                  class="max-w-full mx-auto md:max-w-none h-auto screenshot"
                />
              </div>
              <div
                class="max-w-xl md:max-w-none md:w-full mx-auto md:col-span-7 lg:col-span-6"
              >
                <div class="md:pr-4 lg:pr-12 xl:pr-16">
                  <div
                    class="font-architects-daughter text-xl text-primary-500 mb-2"
                  >
                    Test
                  </div>
                  <h3 class="h3 mb-3">Test Beschreibung</h3>
                  <p class="text-xl text-gray-700 dark:text-gray-400 mb-4">
                    Hier soll ggf. die minimierte Version von einem Tab
                    erscheinen!
                  </p>
                  <ul class="text-lg text-gray-700 dark:text-gray-400 mb-8">
                    <li class="flex items-center mb-2">
                      <svg
                        viewBox="0 0 12 12"
                        xmlns="http://www.w3.org/2000/svg"
                        class="w-3 h-3 fill-current text-teal-500 mr-2 flex-shrink-0"
                      >
                        <path
                          d="M10.28 2.28L3.989 8.575 1.695 6.28A1 1 0 00.28 7.695l3 3a1 1 0 001.414 0l7-7A1 1 0 0010.28 2.28z"
                        ></path>
                      </svg>
                      <span>TEST</span>
                    </li>
                  </ul>
                  <div
                    class="flex items-center mt-6 text-primary-500 hover:text-primary-600 hover:underline"
                  >
                    <svg class="feather feather-xs">
                      <use
                        xlink:href="/images/svg/feather-sprite.svg#arrow-right-circle"
                      ></use>
                    </svg>
                    <a
                      href="/"
                      title="SwiftDesk - Shared Inboxes"
                      class="ml-2 hover:text-primary-600 hover:underline"
                      >Weiterleitung</a
                    >
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  </div>
  <Fotter></Fotter>
</template>

<script>
import Fotter from "@/components/footer.vue";
import * as url from "url";
export default {
  name: "HomeView",
  computed: {
    url() {
      return url;
    },
  },
  components: {
    Fotter,
  },
};
</script>

<style scoped>
.bgDiv {
  background-image: url("@/assets/beach.jpg");
  background-size: cover;
  background-position: center;
}
</style>
